...
ParallelStencil.ParallelKernel.randn_cuda(::Type{T}, blocklength, args...) where {T<:Union{Number,Enum}}          = CuArray(randn_cpu(T, blocklength, args...))        # ...
...
ParallelStencil.ParallelKernel.rand_cuda(::Type{T}, blocklength, dims...) where {T<:Union{SArray,FieldArray}}    = randn_cuda(T, blocklength, dims)
ParallelStencil.ParallelKernel.randn_cuda(::Type{T}, ::Val{B},    dims)    where {T<:Union{SArray,FieldArray}, B} = (check_datatype_cuda(T, Bool, Enum); blocklen = (B == 0) ? prod(dims) : B; CellArray{T,length(dims),B, CUDA.CuArray{eltype(T),3}}(CUDA.randn(eltype(T), blocklen, prod(size(T)), ceil(Int,prod(dims)/(blocklen))), dims))
ParallelStencil.ParallelKernel.randn_cuda(::Type{T}, blocklength, dims...) where {T<:Union{SArray,FieldArray}}    = randn_cuda(T, blocklength, dims)
